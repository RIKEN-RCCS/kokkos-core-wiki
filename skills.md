...
2. それでも漏れた場合の決定的なバックストップとして `fix_inline_markup_spacing()` を実装済み(`fix_underlines()` と同じ「LLM 出力への非LLM後処理」パターン)。
   ...
   - 加えて、全角括弧(`（「『【〔《〈｛［` / `）」』】〕》〉｝］`)についても、開き記号の直前の全角**閉じ**括弧、および閉じ記号の直後の全角**開き**括弧を検出して同様に `\ ` を挿入する(`_needs_prefix_escape` / `_needs_suffix_escape`。docutils の `start_string_prefix`/`end_string_suffix` は開き記号と閉じ記号を非対称に扱うため、かな・カタカナ・漢字と違って方向ごとに別関数になっている)
...
- 対象は「かな・カタカナ・漢字、および代表的な全角括弧がインライン記法の記号に直接接触するケース」に絞ってある。全角数字や上記以外の全角記号(コロン・句読点以外の約物など)への隣接は意図的にスコープ外(誤検知でコード片を壊すリスクを避けるため)。
