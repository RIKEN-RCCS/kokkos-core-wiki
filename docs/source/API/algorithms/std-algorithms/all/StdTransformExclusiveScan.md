...
- `unary_op`:
  ...
  引数として渡された実行空間から呼び出されるためには、有効でなければならない、そして 型 (可能性のあるconst)　value_type　の引数　`v`　について、bブール型に変換可能で、そこでは、value_type``　が、 `first_from` (1,2について)　の値型、または `view_from` (3,4について)　であり、 `v` 　を変更してはいけません。
