...
- ``g``: すべての要素上で呼び出される関数オブジェクト
  ...
  - 以下の形式のファンクタであり、, ここで、 (1,2,5について) または、 of ``view`` (3,4,6について)　の　``IteratorType``　の値型である、 ``value_type`` を使って、 ``return_type`` は、``value_type``　に割り当て可能でなければなりません。
