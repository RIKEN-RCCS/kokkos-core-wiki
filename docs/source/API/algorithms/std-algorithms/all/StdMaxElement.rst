...
要素の比較には「operator<」を使用するか、ユーザーが提供する比較演算子を使用して、範囲内またはランク1の ``ビュー`` 内で最大の要素を検索します。
...
- ``exespace``, ``first``, ``last``, ``view``, ``comp``: same as in |min_element_link|_　におけるのよび要件
~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
- ``exespace``, ``first``, ``last``, ``view``, ``comp``: same as in |min_element_link|_　におけるのと同様。
