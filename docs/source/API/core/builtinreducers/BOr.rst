...
`ReducerConcept <ReducerConcept.html>`_ のすべての公開型、コンストラクタ、およびメソッドが利用可能です。以下の型とメソッドは、このリデューサーによってオーバーライドされます:
...
``Scalar`` テンプレートパラメータから、潜在的な ``const`` および/または ``volatile`` 修飾子を取り除いたもの。
...
* ``BOr`` をカスタム型と共に使用するには、 ``Kokkos::reduction_identity<CustomType>`` のテンプレート特殊化を定義する必要があります。詳細は `Built-In Reducers with Custom Scalar Types <../../../ProgrammingGuide/Custom-Reductions-Built-In-Reducers-with-Custom-Scalar-Types.html>`_ を参照してください。
