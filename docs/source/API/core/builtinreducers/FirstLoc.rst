...
.. cpp:class:: template<class Index, class Space> FirstLoc
   ...
   .. cpp:type:: value_type
      ...
      リダクションのスカラー型（`FirstLocScalar <FirstLocScalar.html>`_ の特殊化）
   ...
   .. cpp:function:: KOKKOS_INLINE_FUNCTION void join(value_type& dest, const value_type& src) const;
      ...
      ``src`` と ``dest`` のうち最初のインデックスを ``dest`` に格納します: ``dest = (src.min_loc_true < dest.min_loc_true) ? src :dest;`` 。
...
* カスタム型の ``Index`` で ``FirstLoc`` を使用するには、``Kokkos::reduction_identity<CustomType>`` のテンプレート特殊化を定義する必要があります。詳細は `カスタムスカラー型を使用する組み込みリデューサー <../../../ProgrammingGuide/Custom-Reductions-Built-In-Reducers-with-Custom-Scalar-Types.html>`_ を参照してください。