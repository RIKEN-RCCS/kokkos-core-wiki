...
.. cpp:class:: template<class Index, class Space> LastLoc
   ...
   .. cpp:type:: value_type
      ...
      リダクションのスカラー型（`LastLocScalar <LastLocScalar.html>`_ の特殊化）
   ...
   .. cpp:function:: KOKKOS_INLINE_FUNCTION void join(value_type& dest, const value_type& src) const;
      ...
      ``src`` と ``dest`` の最後のインデックスとの最大値を ``dest`` に格納します: ``dest = (src.max_loc_true > dest.max_loc_true) ? src :dest;``。

   .. cpp:function:: KOKKOS_INLINE_FUNCTION void init(value_type& val) const;

      ``Kokkos::reduction_identity<Index>::max()`` メソッドを使用して ``val.max_loc_true`` を初期化します。デフォルトの実装では ``val=<TYPE>_MIN`` に設定されます。

   .. cpp:function:: KOKKOS_INLINE_FUNCTION value_type& reference() const;

      クラスのコンストラクターで提供された結果への参照を返します。

   .. cpp:function:: KOKKOS_INLINE_FUNCTION result_view_type view() const;

      クラスのコンストラクターで提供された結果のビューを返します。
...
* カスタム型の ``Index`` で ``LastLoc`` を使用するには、``Kokkos::reduction_identity<CustomType>`` のテンプレート特殊化を定義する必要があります。詳細は `カスタムスカラー型を用いた組み込みリデューサー <../../../ProgrammingGuide/Custom-Reductions-Built-In-Reducers-with-Custom-Scalar-Types.html>`_ を参照してください。