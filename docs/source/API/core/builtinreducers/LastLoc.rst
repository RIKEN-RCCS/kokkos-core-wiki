...
.. cpp:class:: template<class Index, class Space> LastLoc
   ...
   .. cpp:function:: KOKKOS_INLINE_FUNCTION void join(value_type& dest, const value_type& src) const;
      ...
      ``src`` と ``dest`` の最後のインデックスを持つ最大値を ``dest`` に格納します: ``dest = (src.max_loc_true > dest.max_loc_true) ? src :dest;``。
...
* カスタム型の ``Index`` で ``LastLoc`` を使用するには、``Kokkos::reduction_identity<CustomType>`` のテンプレート特殊化を定義する必要があります。詳細については `カスタムスカラー型を持つ組み込みリデューサー <../../../ProgrammingGuide/Custom-Reductions-Built-In-Reducers-with-Custom-Scalar-Types.html>`_ を参照してください。