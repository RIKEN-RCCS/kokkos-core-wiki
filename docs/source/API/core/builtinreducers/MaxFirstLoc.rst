...
.. cpp:class:: template<class Scalar, class Index, class Space> MaxFirstLoc
   ...
   .. cpp:function:: KOKKOS_INLINE_FUNCTION void join(value_type& dest, const value_type& src) const;
      ...
      ``src`` と ``dest`` のうち最大値とその最初のインデックスを ``dest`` に格納します: ``dest = (src.val > dest.val) ? src :dest;``。
...
* ``Scalar`` または ``Index`` のいずれかにカスタム型を使用して ``MaxFirstLoc`` を利用するには、``Kokkos::reduction_identity<CustomType>`` のテンプレート特殊化を定義する必要があります。詳細は `カスタムスカラー型を用いた組み込みリデューサー <../../../ProgrammingGuide/Custom-Reductions-Built-In-Reducers-with-Custom-Scalar-Types.html>`_ を参照してください。