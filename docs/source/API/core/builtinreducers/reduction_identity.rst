...
.. cpp:struct:: template <typename ScalarType> reduction_identity
   :no-index-entry:
    ...
    様々な縮約型に対する恒等値を返す静的メンバー関数を提供します。

   .. rubric:: 整数型および浮動小数点型の両方で利用可能な静的メンバー:

   .. cpp:function:: KOKKOS_FUNCTION static ScalarType sum()

      :returns: 組み込みリデューサー :cpp:class:`Sum` 用の中立要素
   ...
   .. cpp:function:: KOKKOS_FUNCTION static ScalarType min()
      ...
      :returns: 組み込みリデューサー :cpp:class:`Min` 用の中立要素
...
``reduction_identity`` 構造体は、様々な一般的な縮約演算における  
単位元（中立要素とも呼ばれる）を提供します。並列縮約において、恒等要素は各スレッドにおける
縮約変数の開始値となる。
他の値と縮約演算を用いて組み合わせた場合、
他の値を変化させません。
例えば、和の縮約については、:math:`x+0=x` であるため、恒等は、 :math:`0` です。プロダクト縮約については、 、  
...
スレッドローカルな縮約アキュムレータを初期化します。

Kokkosは、:cpp:`complex<T>` だけでなく、すべての算術型（つまり整数型および浮動小数点型）
に対して特殊化を提供します。
...
静的メンバー関数を提供しなければならない。 これらの関数は、適切な識別値で初期化された
...
.. code-block:: cpp
    ...
    struct Kokkos::reduction_identity<sample::ValueType> {
      KOKKOS_FUNCTION static sample::ValueType sum() {
        return sample::ValueType(); // デフォルトコンストラクタは、0に初期化します。
      }
