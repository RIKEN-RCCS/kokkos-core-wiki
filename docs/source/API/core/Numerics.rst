...
ヘッダー ``<Kokkos_MathematicalConstants.hpp>`` は、 C++20 標準ライブラリヘッダー ``<numbers>`` のバックポートであり、 ``pi`` または ``sqrt2`` 等いくつかの数学定数を提供します。
...
ヘッダー ``<Kokkos_Half.hpp>`` は、16ビットの「半精度」型 ``half_t`` および「ブレイン浮動小数点」型 ``bhalf_t`` の浮動小数点を実装します。
