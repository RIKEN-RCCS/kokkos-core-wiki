...
.. |ExecutionSpaceConceptType| 置換:: :cpp:func:`ExecutionSpace` type

.. _DocExecutionSpaceConcept: #kokkos-executionspaceconcept

.. |DocExecutionSpaceConcept| 置換:: the documentation on the :cpp:func:`ExecutionSpace` concept

.. _Experimental: utilities/experimental.html#experimentalnamespace

.. |Experimental| 置換:: Experimental

.. _KokkosConcepts: KokkosConcepts.html

.. |KokkosConcepts| 置換:: this document
...
.. code-block:: cpp
    ...
    template<class MS>
    構造体 is_execution_space {
    enum { value = false };
    };

    template<>
    構造体 is_execution_space<ExecutionSpaceConcept> {
    enum { value = true };
    };
...
* ``ExecutionSpaceConcept(const ExecutionSpaceConcept& src)``: Copy constructor.コピーコンストラクタ。
