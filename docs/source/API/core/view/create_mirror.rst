...
``create_mirror_view_and_copy(typename ViewType::host_mirror_type::memory_space{}, src)`` を呼び出すことと等価です。つまり、元の |View|_ と同じ値を含むホストアクセス可能な View を返します。もし |View|_ がすでにホストアクセス可能であれば、元の |View|_ の (浅い) コピーが返されます。
