...
指定された実行空間が ``src`` と ``dest`` にアクセスできる場合（3引数のオーバーロードの場合）、または、ソースもしくはデスティネーションのメモリ空間に対応する実行空間が両方のビューにアクセスできる場合、deep_copy() は要素ごとに代入することで値型や異なるレイアウトを変換できます。それ以外の場合は、memcopy が実行されるため、値型とレイアウトは同一かつ連続していなければなりません。
...
- 以下の条件セットのいずれかが真でなければなりません:

- ``std::is_assignable_v<ViewSrc::value_type, DestView::value_type>`` であり、かつ ``SpaceAccessibility<copy_space, ViewDest::memory_space>::accessible == true`` と ``SpaceAccessibility<copy_space,ViewSrc::memory_space>::accessible == true`` の両方が成り立つような `ExecutionSpace <../execution_spaces.html>`_ ``copy_space`` （指定またはデフォルト）が存在すること。
...
デバイスビューから互換性のないホストビューへデータを移動するには、一時的な割り当てを使用して2つのステップで操作を行う必要があります:
