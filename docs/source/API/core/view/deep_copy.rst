...
指定された実行空間が ``src`` および ``dest`` にアクセス可能な場合（3引数のオーバーロードの場合）、またはソースやデスティネーションのメモリ空間に対応する実行空間が両方のビューにアクセス可能な場合、deep_copy() はエントリごとに代入することで、値型の変換や異なるレイアウトの変換を行えます。それ以外の場合、memcopy が実行されるため、値型とレイアウトは同一かつ連続している必要があります。
...
- 次の条件セットのいずれかが真でなければなりません:

- ``std::is_assignable_v<ViewSrc::value_type, DestView::value_type>`` であり、かつ ``SpaceAccessibility<copy_space, ViewDest::memory_space>::accessible == true`` および ``SpaceAccessibility<copy_space,ViewSrc::memory_space>::accessible == true`` の両方が成り立つような `ExecutionSpace <../execution_spaces.html>`_ ``copy_space`` （指定またはデフォルト）が存在すること。

- または ``src.span_is_contiguous() && dest.span_is_contiguous()``、 ``std::is_same_v<ViewDest::array_layout,ViewSrc::array_layout>``、そして ``std::is_same_v<DestView::value_type, SrcView::non_const_value_type>`` であること。
