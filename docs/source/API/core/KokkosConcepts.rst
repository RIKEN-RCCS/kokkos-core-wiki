...
Some things currently being treated as concepts (according to ``Kokkos_Concepts.hpp``) that are really more like enumerations (or something similar) are:現在、一部の項目は、概念として扱われているものの（　``Kokkos_Concepts.hpp`` によると）、実際には列挙型（または類似のもの）に近い性質を持つものは以下の通りです:
