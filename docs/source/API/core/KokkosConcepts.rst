...
Kokkos は、創業当初から本質的にコンセプト主導のデザインを採用してきました。
...
近い将来に、SYCL、レジリエンス、そして　HPX　など、複数の新しいバックエンドプロジェクトが控えており、いくつか例を挙げると、今こそが、　Kokkos　の中核概念を正式に定義し、汎用コードで Kokkos が使用する概念セットを反復的に強化し始める最適な時期です。
...
.. |ExecutionPolicy| 置換:: ``ExecutionPolicy``

.. _RangePolicy: policies/RangePolicy.html

.. |RangePolicy| 置換:: ``RangePolicy``

.. _TeamMember: policies/TeamHandleConcept.html

.. |TeamMember| 置換:: ``TeamMember``

認知負荷に関して言えば、概念の総数を制限することよりも、おそらくさらに重要なのは、概念の　*包含的階層*　の数を制限することです。　C++　の範囲に関する経験からも、
...
現在、一部の項目は、概念として扱われているものの（　``Kokkos_Concepts.hpp`` によると）、実際には列挙型（または類似のもの）に近い性質を持つものは以下の通りです:

* ``MemoryTraits``
* 実行ポリシーでのみ使用されるいくつかの項目:
    - ``Schedule``
    - The ``IndexType<>`` tag
...
.. code-block:: cpp
    ...
    概念 ExecutionSpaceOf =
        ...
        // Kokkos_Parallel.hpp　により課される必要要件
        requires(Ex ex, ExPol const& policy, F f, ResultType& total) {
            // 厳密に言えば、技術的には正しくないのですが、
            // 簡潔さを優先して申し上げますと、実行メソッドはこれらの要件を満たすものの、
            // Kokkosでは動作しません：
    ...
    概念 ExecutionSpaceOfReduction =
        ExecutionSpaceOf<Ex, ExPol, F> &&
        Reducer<Red> &&
        // Kokkos_Parallel_Reduce.hpp　によって課された必要要件
        requires(
...
現在の実装における ``CudaSpace``、 ``CudaUVMSpace``、``HostSpace`` および ``OpenMPTargetSpace``　の共通機能を見れば、 ``MemorySpace`` の現在の概念は、以下の様に見えます:
