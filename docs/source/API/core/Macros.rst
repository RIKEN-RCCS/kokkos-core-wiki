マクロ
======

関数マークアップマクロ
----------------------

.. toctree::
   :maxdepth: 1

   macros-special/host_device_macros.rst

``KOKKOS_IF_ON_HOST`` and ``KOKKOS_IF_ON_DEVICE``
-------------------------------------------------

.. toctree::
   :maxdepth: 1

   macros-special/if_on_host_or_device.rst

バージョンマクロ
----------------

.. list-table::
   :header-rows: 1
   :align: left

   * - バージョン整数マクロ
     -  Kokkos バージョン 4.2.1を想定した値の例 
   * - ``KOKKOS_VERSION``
     - ``40201``
   * - ``KOKKOS_VERSION_MAJOR``
     - ``4``
   * - ``KOKKOS_VERSION_MINOR``
     - ``2``
   * - ``KOKKOS_VERSION_PATCH``
     - ``1``

バージョンマクロは、整数として定義されます。
``KOKKOS_VERSION`` は、 ``<MAJOR>*10000 + <MINOR>*100 + <PATCH>`` に等しいです。
``40201`` は、 Kokkos 4.2.1 リリースバージョンに対応します。
``99`` パッチ番号は、開発版を示します。 つまり、 ``40199`` は、
Kokkos 開発バージョン ポストリリース 4.1 に対応します。

.. code-block:: c++
   
   #include <Kokkos_Core.hpp>
   #include <iostream>
   static_assert(KOKKOS_VERSION_MAJOR == KOKKOS_VERSION / 10000);
   static_assert(KOKKOS_VERSION_MINOR == KOKKOS_VERSION / 100 % 100);
   static_assert(KOKKOS_VERSION_PATCH == KOKKOS_VERSION % 100);
   #if KOKKOS_VERSION >= 30700
   // Kokkos バージョンは、少なくとも 3.7
   #endif
   int main() {
     if (KOKKOS_VERSION_PATCH == 99)
       std::cout << "Kokkos の開発バージョンを使用"
     // ...
   }

.. warning:: Kokkos 4.1まで、 ``KOKKOS_VERSION_MINOR`` および ``KOKKOS_VERSION_PATCH`` は、それらが本来 ``0`` であるべき時に、定義されませんでした。

.. list-table::
   :header-rows: 1
   :align: left

   * - 関数のようなバージョンのヘルパーマクロ ( Kokkos 4.1以降)
   * - ``KOKKOS_VERSION_LESS(メジャー・マイナー・パッチ)``
   * - ``KOKKOS_VERSION_LESS_EQUAL(メジャー・マイナー・パッチ)``
   * - ``KOKKOS_VERSION_GREATER(メジャー・マイナー・パッチ)``
   * - ``KOKKOS_VERSION_GREATER_EQUAL(メジャー・マイナー・パッチ)``
   * - ``KOKKOS_VERSION_EQUAL(メジャー・マイナー・パッチ)``

``KOKKOS_VERSION_<COMPARE>`` 関数のようなマクロは、現在使用中の Kokkos のバージョンと、3つの引数 ``major.minor.patch`` で指定されたバージョンを比較した結果を返します。 これらは、Kokkos
4.1から利用可能です。

.. code-block:: c++
   
   #include <Kokkos_Core.hpp>
   // イラストレーションの整合性チェック
   static_assert(KOKKOS_VERSION_EQUAL(KOKKOS_VERSION_MAJOR,
                                      KOKKOS_VERSION_MINOR,
                                      KOKKOS_VERSION_PATCH));
   // static_assert(KOKKOS_VERSION_GREATER_EQUAL(4, 5, 0)) プロジェクトまたはファイルに対して、Kokkos の最小必要バージョンを設定します;

   void do_work() {
     #if KOKKOS_VERSION_GREATER_EQUAL(4, 3, 0)
     // 新しいrad機能の使用
     #else
     // 昔ながらの退屈なものへの後退
     #endif
   }

一般設定
--------

+-------------------------------------------------+--------------------------------------------------------------------------------------------------------------------+
| マクロ                                          | 説明                                                                                                               |
+=================================================+====================================================================================================================+
| ``KOKKOS_ENABLE_DEBUG``                         | 追加のデバッグ機能が有効化されているかどうかを定義。                                                               |
+-------------------------------------------------+--------------------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ENABLE_DEBUG_BOUNDS_CHECK``            | 追加の境界チェックが有効化されているかどうかを定義。                                                               |
+-------------------------------------------------+--------------------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ENABLE_DEBUG_DUALVIEW_MODIFY_CHECK``   | デバッグチェックで　``Kokkos::DualView``　オブジェクトの有効化が有効かどうかを定義                                 |
+-------------------------------------------------+--------------------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ENABLE_DEPRECATED_CODE_3``             | メジャーリリース　3　で非推奨となった機能がまだ利用可能かどうかを定義。                                            |
+-------------------------------------------------+--------------------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ENABLE_DEPRECATION_WARNING``           | 非推奨機能が非推奨警告を生成するかどうかを定義。                                                                   |
+-------------------------------------------------+--------------------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ENABLE_TUNING``                        | チューニング用のバインディングが利用可能かどうか ( `#2422 <https://github.com/kokkos/kokkos/pull/2422>`_ を参照)。 |
+-------------------------------------------------+--------------------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ENABLE_COMPLEX_ALIGN``                 | 複合型が配置されるかどうか。                                                                                       |
+-------------------------------------------------+--------------------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ENABLE_AGGRESSIVE_VECTORIZATION``      | 内部　Kokkos　ループの積極的なベクトル化において、特定の依存性仮定が無視されるかどうか。                           |
+-------------------------------------------------+--------------------------------------------------------------------------------------------------------------------+

実行空間
--------

以下のマクロを使用すると、指定された実行スペースが有効かどうかをテストできます。 それらの存在を確認可能です (例えば、 ``#ifdef KOKKOS_ENABLE_SERIAL``)。

.. _Serial: execution_spaces.html

.. |Serial| replace:: :cpp:func:`Serial`

.. _OpenMP: execution_spaces.html

.. |OpenMP| replace:: :cpp:func:`OpenMP`

.. _Threads: execution_spaces.html

.. |Threads| replace:: :cpp:func:`Threads`

.. _Cuda: execution_spaces.html

.. |Cuda| replace:: :cpp:func:`Cuda`

.. _HPX: execution_spaces.html

.. |HPX| replace:: :cpp:func:`HPX`

+--------------------------------+----------------------------------------------------------+
| マクロ                         | 説明                                                     |
+================================+==========================================================+
| ``KOKKOS_ENABLE_SERIAL``       | |Serial|_ 実行空間が有効かどうかを定義。                 |
+--------------------------------+----------------------------------------------------------+
| ``KOKKOS_ENABLE_OPENMP``       | |OpenMP|_ 実行空間が有効かどうかを定義。                 |
+--------------------------------+----------------------------------------------------------+
| ``KOKKOS_ENABLE_THREADS``      | |Threads|_ 実行空間が有効かどうかを定義。                |
+--------------------------------+----------------------------------------------------------+
| ``KOKKOS_ENABLE_CUDA``         | |Cuda|_ 実行空間が有効かどうかを定義。                   |
+--------------------------------+----------------------------------------------------------+
| ``KOKKOS_ENABLE_HIP``          | 実験 ``HIP`` 実行空間が有効かどうかを定義。              |
+--------------------------------+----------------------------------------------------------+
| ``KOKKOS_ENABLE_HPX``          | |HPX|_ 実行空間が有効かどうかを定義。                    |
+--------------------------------+----------------------------------------------------------+
| ``KOKKOS_ENABLE_SYCL``         | 実験 ``SYCL`` 実行空間が有効かどうかを定義。             |
+--------------------------------+----------------------------------------------------------+

バックエンドオプション
----------------------

+-------------------------------------------------------+-------------------------------------------------------------------------------------------------------------+
| マクロ                                                | 説明                                                                                                        |
+=======================================================+=============================================================================================================+
| ``KOKKOS_ENABLE_CUDA_CONSTEXPR``                      | CUDA バックエンドがconstexpr関数をサポートするかどうかを定義。                                              |
+-------------------------------------------------------+-------------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ENABLE_CUDA_LAMBDA``                         | CUDA バックエンドがラムダをサポートするかどうかを定義。                                                     |
+-------------------------------------------------------+-------------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ENABLE_CUDA_LDG_INTRINSICS``                 | CUDA バックエンドがLDG イントリンシックをサポートするかどうかを定義。                                       |
+-------------------------------------------------------+-------------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ENABLE_CUDA_RELOCATABLE_DEVICE_CODE``        | CUDA バックエンドが再配置可能デバイスコードをサポートするかどうかを定義。                                   |
+-------------------------------------------------------+-------------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ENABLE_CUDA_UVM``                            | 定義されている場合、デフォルトの CUDA メモリ空間はCudaUVMSpaceであり、そうでない場合には、それは CudaSpace。|
+-------------------------------------------------------+-------------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ENABLE_HIP_MULTIPLE_KERNEL_INSTANTIATIONS``  | 定義されている場合、複数のカーネルバージョンがインスタンス化され、実行時間が改善される可能性あり。          |
+-------------------------------------------------------+-------------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ENABLE_HIP_RELOCATABLE_DEVICE_CODE``         | バックエンドが再配置可能デバイスコードをサポートするかどうかを定義。                                        |
+-------------------------------------------------------+-------------------------------------------------------------------------------------------------------------+

C++ 標準設定
------------

Kokkos は最新のC++言語標準をサポートしています。 特定の機能は、有効になっている言語標準に応じて、異なるコードパスを使用したり、異なるコンパイラサポートを持つ場合があります。Kokkos がどの言語標準でコンパイルされたかを判別するために公開されています。

+-------------------------+---------------------------------------+
| マクロ                  | 説明                                  |
+=========================+=======================================+
| ``KOKKOS_ENABLE_CXX14`` |  C++14 言語標準が有効化されています。 |
+-------------------------+---------------------------------------+
| ``KOKKOS_ENABLE_CXX17`` |  C++17 言語標準が有効化されています。 |
+-------------------------+---------------------------------------+
| ``KOKKOS_ENABLE_CXX20`` |  C++20 言語標準が有効化されています。 |
+-------------------------+---------------------------------------+

サードパーティライブラリの設定
------------------------------

これらの定義は、 Kokkos がコンパイルされたサードパーティライブラリに関する情報を提供します。

+-------------------------------+----------------------------------------------------------------------------------------------------------------------------+
| マクロ                        | 説明                                                                                                                       |
+===============================+============================================================================================================================+
| ``KOKKOS_ENABLE_HWLOC``       | NUMA およびアーキテクチャ情報用に `libhwloc <https://www.open-mpi.org/projects/hwloc/>`_ が有効化されているかどうかを定義。|
+-------------------------------+----------------------------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ENABLE_LIBDL``       | Kokkosが動的リンカー（libdl）にリンクしているかどうかを定義。                                                              |
+-------------------------------+----------------------------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ENABLE_LIBQUADMATH`` | Kokkosが `GCC Quad-Precision Math Library API <https://gcc.gnu.org/onlinedocs/libquadmath/>`_ にリンクするかどうかを定義。 |
+-------------------------------+----------------------------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ENABLE_ONEDPL``      | SYCLバックエンド使用時にKokkosが `oneDPLライブラリ <https://github.com/oneapi-src/oneDPL>`_ にリンクするかどうかを定義。   |
|                               | このTPLを有効にすると、それを使用するKokkosアルゴリズム（例：`sort`）のパフォーマンスに影響を与える可能性があり。          |
+-------------------------------+----------------------------------------------------------------------------------------------------------------------------+

アーキテクチャ
--------------

+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| マクロ                            | 説明                                                                                                   |
+===================================+========================================================================================================+
| ``KOKKOS_ARCH_ARMV80``            | ARMv8.0 互換 CPU (HOST) 向けに最適化。                                                                 |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_ARMV8_THUNDERX``    | ARMv8 Cavium ThunderX CPU (HOST) 向けに最適化。                                                        |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_ARMV81``            | ARMv8.1 互換 CPU (HOST) 向けに最適化。                                                                 |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_ARMV8_THUNDERX2``   | ARMv8 Cavium ThunderX2 CPU (HOST) 向けに最適化。                                                       |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AMD_AVX2``          | AVX2 の最適化（Zen で有効化）。                                                                        |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AMD_GFX906``        | AMD GPU MI50、MI60（GPU）向けに最適化。                                                                |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AMD_GFX908``        | AMD GPU MI100 (GPU) 向けに最適化。                                                                     |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AMD_GFX90A``        | AMD GPU MI200 シリーズ（GPU）向けに最適化。                                                            |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AMD_GFX940``        | AMD GPU MI300A 試作版向けに最適化 (GPU)。                                                              |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AMD_GFX942``        | AMD GPU MI300X (GPU) 向けに最適化。                                                                    |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AMD_GFX942_APU``    | AMD GPU MI300A (GPU) 向けに最適化。                                                                    |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AMD_GFX1030``       | AMD GPU V620、W6800 (GPU) 向けに最適化。                                                               |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AMD_GFX1100``       | AMD GPU 7900 XT (GPU) 向けに最適化。                                                                   |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AMD_GFX1103``       | AMD Phoenix APU（GPU）向けに最適化。                                                                   |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AMD_GFX1201``       | AMD GPU 9070 XT (GPU) 向けに最適化。                                                                   |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AMD_GPU``           | AMD GPU のアーキテクチャフラグの文字列（例：MI250X アクセラレータをターゲットとする場合 "gfx90A"）     |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AVX``               | AVX の最適化。                                                                                         |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AVX2``              | AVX2 の最適化。                                                                                        |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AVX512XEON``        | Skylake(AVX512) の最適化。                                                                             |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_KNC``               | インテル Knights Corner Xeon Phi (HOST) 向けに最適化。                                                 |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AVX512MIC``         | Many Integrated Core (MIC; AVX512) 向けに最適化。                                                      |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_POWER8``            | IBM POWER8 CPU 向けに最適化 (HOST)。                                                                   |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_POWER9``            | IBM POWER9 CPU 向けに最適化 (HOST)。                                                                   |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_RISCV_SG2042``      | RISC-V SG2042 CPU 向けに最適化 (HOST)。                                                                |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_RISCV_RVA22V``      | RISC-V RVA22V CPU 向けに最適化 (HOST)。                                                                |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_RISCV_U74MC``       | RISC-V U74MC CPU 向けに最適化 (HOST)。                                                                 |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_INTEL_GEN``         | インテル GPU、ジャストインタイムコンパイル向けに最適化（GPU）。                                        |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_INTEL_DG1``         | Intel Iris XeMAX GPU (GPU) 向けに最適化。                                                              |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_INTEL_DG2``         | Intel DG2 GPU (GPU) 向けに最適化。                                                                     |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_INTEL_GEN9``        | Intel GPU Gen9 (GPU) 向けに最適化。                                                                    |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_INTEL_GEN11``       | Intel GPU Gen11 (GPU) 向けに最適化。                                                                   |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_INTEL_GEN12LP``     | Intel GPU Gen12LP (GPU) 向けに最適化。                                                                 |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_INTEL_XEHP``        | Intel GPU Xe-HP (GPU) 向けに最適化。                                                                   |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_INTEL_PVC``         | Intel GPU Ponte Vecchio/GPU Max (GPU) 向けに最適化。                                                   |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_INTEL_GPU``         | いずれかの Intel GPU アーキテクチャが有効化されているかどうかを設定。                                  |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_KEPLER``            | いずれかの NVIDIA Kepler アーキテクチャが有効化されているかどうかを設定。                              |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_KEPLER30``          | NVIDIA Kepler generation CC 3.0 (GPU) 向けに最適化。                                                   |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_KEPLER32``          | NVIDIA Kepler generation CC 3.2 (GPU) 向けに最適化。                                                   |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_KEPLER35``          | NVIDIA Kepler generation CC 3.5 (GPU) 向けに最適化。                                                   |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_KEPLER37``          | NVIDIA Kepler generation CC 3.7 (GPU) 向けに最適化。                                                   |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_MAXWELL``           | いずれかの NVIDIA Maxwell アーキテクチャが有効化されているかどうかを設定。                             |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_MAXWELL50``         | NVIDIA Maxwell generation CC 5.0 (GPU) 向けに最適化。                                                  |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_MAXWELL52``         | NVIDIA Maxwell generation CC 5.2 (GPU) 向けに最適化。                                                  |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_MAXWELL53``         | NVIDIA Maxwell generation CC 5.3 (GPU) 向けに最適化。                                                  |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_NAVI``              | いずれかの AMD Navi GPU アーキテクチャが有効化されているかどうかを設定。:sup:`Kokkos 4.0 以降`         |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_NAVI1030``          | AMD GPU V620/W6800 GFX1030 (GPU) 向けに最適化。:sup:`Kokkos 4.0 以降`                                  |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_PASCAL``            | いずれかの NVIDIA Pascal アーキテクチャが有効化されているかどうかを設定。                              |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_PASCAL60``          | NVIDIA Pascal generation CC 6.0 (GPU) 向けに最適化。                                                   |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_PASCAL61``          | NVIDIA Pascal generation CC 6.1 (GPU) 向けに最適化。                                                   |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_VOLTA``             | いずれかの NVIDIA Volta アーキテクチャが有効化されているかどうかを設定。                               |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_VOLTA70``           | NVIDIA Volta generation CC 7.0 (GPU) 向けに最適化。                                                    |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_VOLTA72``           | NVIDIA Volta generation CC 7.2 (GPU) 向けに最適化。                                                    |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_TURING75``          | NVIDIA Turing generation CC 7.5 (GPU) 向けに最適化。                                                   |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AMPERE``            | いずれかの NVIDIA Ampere アーキテクチャが有効化されているかどうかを設定。                              |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AMPERE80``          | NVIDIA Ampere generation CC 8.0 (GPU) 向けに最適化。                                                   |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AMPERE86``          | NVIDIA Ampere generation CC 8.6 (GPU) 向けに最適化。                                                   |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AMPERE87``          | NVIDIA Ampere generation CC 8.7 (GPU) 向けに最適化。                                                   |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_ADA89``             | NVIDIA Ada generation CC 8.9 (GPU) 向けに最適化。:sup:`Kokkos 4.1 以降`                                |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_HOPPER``            | いずれかの NVIDIA Hopper アーキテクチャが有効化されているかどうかを設定。:sup:`Kokkos 4.0 以降`        |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_HOPPER90``          | NVIDIA Hopper generation CC 9.0 (GPU) 向けに最適化。:sup:`Kokkos 4.0 以降`                             |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_BLACKWELL``         | いずれかの NVIDIA Blackwell アーキテクチャが有効化されているかどうかを設定。:sup:`Kokkos 4.7 以降`     |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_BLACKWELL100``      | NVIDIA Blackwell generation CC 10.0 (GPU) 向けに最適化。:sup:`Kokkos 4.7 以降`                         |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_BLACKWELL103``      | NVIDIA Blackwell generation CC 10.3 (GPU) 向けに最適化。:sup:`Kokkos 5.1 以降`                         |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_BLACKWELL120``      | NVIDIA Blackwell generation CC 12.0 (GPU) 向けに最適化。:sup:`Kokkos 4.7 以降`                         |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AMD_ZEN``           | AMD Zen アーキテクチャ (HOST) 向けに最適化。                                                           |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AMD_ZEN2``          | AMD Zen2 アーキテクチャ (HOST) 向けに最適化。                                                          |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AMD_ZEN3``          | AMD Zen3 アーキテクチャ (HOST) 向けに最適化。                                                          |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AMD_ZEN4``          | AMD Zen4 アーキテクチャ (HOST) 向けに最適化。                                                          |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_AMD_ZEN5``          | AMD Zen5 アーキテクチャ (HOST) 向けに最適化。                                                          |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_VEGA``              | いずれかの AMD Vega GPU アーキテクチャが有効化されているかどうかを設定。                               |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_VEGA900``           | AMD GPU MI25 GFX900 (GPU) 向けに最適化。:sup:`Kokkos 4.0 において削除`                                 |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_VEGA906``           | AMD GPU MI50/MI60 GFX906 (GPU) 向けに最適化。                                                          |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_VEGA908``           | AMD GPU MI100 GFX908 (GPU) 向けに最適化。                                                              |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
| ``KOKKOS_ARCH_VEGA90A``           | AMD GPU MI200 シリーズ GFX90A (GPU) 向けに最適化。                                                     |
+-----------------------------------+--------------------------------------------------------------------------------------------------------+
