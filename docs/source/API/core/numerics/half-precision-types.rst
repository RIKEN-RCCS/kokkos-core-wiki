...
マクロ ``KOKKOS_HALF_T_IS_FLOAT`` および ``KOKKOS_BHALF_T_IS_FLOAT`` は、 ``half_t`` および ``bhalf_t`` が ``float`` にマッピングされる場合には ``true`` に、そうでない場合には ``false`` に設定されます。
...
³ --std=c++20 (https://docs.nvidia.com/cuda/archive/12.3.2/cuda-toolkit-release-notes/index.html#cuda-math-release-12-3) を使ってコンパイルする際、nvcc-12.2 に対しては該当しません。
