...
.. [#not_available_with_sycl] (SYCLでは利用不可)
...
* - |exp|_
     - :math:`e` を指定された指数でべき乗した値を返す (:math:`e^x`)
   * - |exp2|_
     - :math:`2` を指定された指数でべき乗した値を返す (:math:`2^x`)
   * - |expm1|_
     - :math:`e` を指定された指数でべき乗した値から :math:`1` を引いた値を返す (:math:`e^x-1`)
   * - |log|_
...
* - |sin|_
   ...
   * - |tan|_
     - タンジェントを計算する (:math:`\tan(x)`)
   * - |asin|_
     - アークサインを計算する (:math:`\arcsin(x)`)
   * - |acos|_
     - アークコサインを計算する (:math:`\arccos(x)`)
   * - |atan|_
     - アークタンジェントを計算する (:math:`\arctan(x)`)
   * - |atan2|_
...
* - |sinh|_
     - 双曲線サインを計算する (:math:`\sinh(x)`)
   * - |cosh|_
     - 双曲線コサインを計算する (:math:`\cosh(x)`)
   * - |tanh|_
     - 双曲線タンジェントを計算する (:math:`\tanh(x)`)
   * - |asinh|_
     - 逆双曲線サインを計算する (:math:`\text{arsinh}(x)`)
   * - |acosh|_
     - 逆双曲線コサインを計算する (:math:`\text{arcosh}(x)`)
   * - |atanh|_
     - 逆双曲線タンジェントを計算する (:math:`\text{artanh}(x)`)
...
* - |ceil|_
   ...
   * - |rint|_ [#since_kokkos_5_1]_
     ...
     - 現在の丸めモードを使用した最も近い整数、結果が異なる場合は例外を発生させる
...
* - |fpclassify|_ [#since_kokkos_5_1]_
   ...
   * - |isnormal|_ [#since_kokkos_5_1]_
     - 指定された数が正規化数かどうかを確認する
   * - |signbit|_
     - 指定された数が負かどうかを確認する
   * - |isgreater|_ [#since_kokkos_5_1]_
     - 第1の浮動小数点引数が第2のものより大きいかどうかを確認する
   * - |isgreaterequal|_ [#since_kokkos_5_1]_
     - 第1の浮動小数点引数が第2のもの以上かどうかを確認する
   * - |isless|_ [#since_kokkos_5_1]_
     - 第1の浮動小数点引数が第2のものより小さいかどうかを確認する
   * - |islessequal|_ [#since_kokkos_5_1]_
     - 第1の浮動小数点引数が第2のもの以下かどうかを確認する
   * - |islessgreater|_ [#since_kokkos_5_1]_
     - 第1の浮動小数点引数が第2のものより小さいまたは大きいかどうかを確認する
...
* **現在実装されていない関数が必要な場合は、遠慮なく** |openIssue|_ **してください。** |issue4767|_ **がこれらを記録しており、実装可能性に関するメモを掲載しています。**
* using ディレクティブ ``using namespace Kokkos;`` は、数学関数への非修飾呼び出しでコンパイルエラーを引き起こすことに注意してください。代わりに、明示的な修飾 (``Kokkos::sqrt``) または using 宣言 (``using
  Kokkos::sqrt;``) を使用してください。( |KnownIssues|_ を参照)
* 数学関数はバージョン4.3で ``Kokkos::Experimental::`` 名前空間から削除されました。
* 四倍精度浮動小数点 ``__float128`` のサポートは、``-DKokkos_ENABLE_LIBQUADMATH=ON`` を介して有効化できます。
