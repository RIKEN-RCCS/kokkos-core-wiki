...
Kokkos 実行環境の現在の状態をクエリできるようにします。Kokkos は厳密な線形の
ライフサイクルに従うため、これらの関数は初期化や終了処理がちょうど一度だけ
実行されるようにするためによく使用されます。
...
.. cpp:function:: bool is_finalized() noexcept
   ...
   :return: Kokkos 実行環境が :cpp:func:`finalize` を通じてシャットダウンされて
     いる場合は ``true``。
...
.. note:: **ライフサイクルの状態機械**
  ...
  Kokkos は 3 つの異なる状態を経ます。Kokkos が一度終了処理されると、同じプロセス
  実行内で **再初期化できない** ことに注意してください。
...
.. caution::
   ...
   **MPI との比較:** MPI に慣れているユーザーは重要な違いに注意してください。
   MPI では、``MPI_Initialized`` は ``MPI_Finalize`` が呼び出された後でも
   ``true`` を返します。Kokkos では、``is_initialized()`` は終了処理後に
   ``false`` を返します。

   ``Kokkos::initialize()`` が今までに呼び出されたかどうかを確認するには、
   次のロジックを使用します:
