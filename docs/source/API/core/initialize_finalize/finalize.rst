...
解放します。
この関数が呼び出されると、いかなる Kokkos API 関数
(:cpp:func:`initialize` さえも) を呼び出すことはできません。ただし、
:cpp:func:`is_initialized` または :cpp:func:`is_finalized` を除きます。
ユーザーは、 ``finalize`` が呼び出される前に、すべての Kokkos オブジェクト (例えば、 :cpp:class:`View`)
が破壊されていることを確認する必要があります。
...
:doc:`ScopeGuard`
    初期化と終了が正しく処理されることを保証するための RAII ベースのアプローチ。
