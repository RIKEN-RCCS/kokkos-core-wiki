...
それは ``argv`` から削除され、 ``argc`` が、1 減算される。
...
`Kokkos::InitializationSettings <InitializationSettings.html#kokkosInitializationSettings>`_ は、 ``Kokkos::InitArguments`` :sup:`バージョン3.7において非推奨` から暗黙的に構築できる。
...
.. code-block:: cpp
    ...
    Kokkos::initialize(InitArguments const& arguments);          // (バージョン 3.7まで) (2)
    Kokkos::initialize(InitializationSettings const& settings);  // (バージョン 3.7以降) (3)
    
パラメータ
~~~~~~~~~~

* ``argc``: 非負の値で、プログラムに渡されたコマンドライン引数の数を表します。
...
* ``settings``: Kokkos の初期化を制御する設定を含む ``class`` オブジェクト。
...
* ユーザーが開始した Kokkos オブジェクトは、 ``Kokkos::initialize`` が呼び出されるまで構築できません。
* ``Kokkos::initialize`` は ``Kokkos::finalize`` の呼び出し後に呼び出すことはできません。

セマンティクス
~~~~~~~~~~~~~~

* ``Kokkos::initialize`` 呼出し後に、 :cpp:func:`Kokkos::is_initialized() <is_initialized()>` は、真 を返す必要があります。
