...
このマクロは、CUDA　および　HIP　における　``__host__ __device__ inline``　マークアップに相当します。
...
注意してください。 それはデフォルトでインラインになるため、単に冗長なだけです。
...
コードの肥大化により実行時のパフォーマンスも低下する恐れがあります。 一部の例においては、``KOKKOS_FORCEINLINE_FUNCTION``　を過剰に使用すると、コンパイラ固有の最大インライン制限によりコンパイルエラーが発生することさえあります。
...
このマクロは、Kokkosがホスト実行スペースのみで構成されている場合、または　CUDA、HIP、SYCL　バックエンドで
...
.. 警告:: ``KOKKOS_FUNCTION`` などのマークが付いた関数内や、``KOKKOS_LAMBDA`` でマークされたラムダ式内で ``KOKKOS_LAMBDA`` を使用しないでください。 特に、ネストされた並列呼び出し用のラムダ式を定義する際には、　``KOKKOS_LAMBDA``　を使用しないでください。CUDAはこれをサポートしていません。代わりに通常のC++構文を使用してください：``[=] (int i) {...}``。

.. 警告:: クラスメンバ関数内でラムダ式を作成する場合、代わりに　``KOKKOS_CLASS_LAMBDA``　を使用する必要がある場合があります。
...
本マクロは、ユーザーにより定義された演鐸ガイドの注釈用に使用されます。
