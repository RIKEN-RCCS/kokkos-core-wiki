...
すべての環境で移植可能というわけではありません。
OpenACC をサポートするコンパイラ等、GPUアクセラレーションコード向けのその他の現代的なコンパイラでは、
ホストコードとデバイスコードの両方を、
...
``KOKKOS_IF_ON_HOST`` および ``KOKKOS_IF_ON_DEVICE`` のコードの差別化の手段としては、
...
これらのマクロの使用は、ホストとデバイスの API 間に根本的な差異が存在し、
