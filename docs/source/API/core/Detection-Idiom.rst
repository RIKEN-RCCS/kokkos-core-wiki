...
.. attention::
   C++20 以前は、Detection Idiom は組み込み typedef の検出や C++ 式の妥当性検出のための最良の仕組みでした。C++20 で追加された言語機能である Concepts は、Detection Idiom よりも優れており、かつ使いやすいため、今後は最初に検討すべき手法です。
...
コピー代入の戻り値の型が ``T&`` であることも確認したい場合は、以下を使用します:
...
.. important::
   Kokkos と C++ 標準ライブラリの両方が、すでに多くの concept を定義しています。独自に作成するよりも、それらを使用することを推奨します。標準化されているだけでなく、コーナーケースを網羅する点でも厳密です。標準ライブラリが提供する concept は <https://eel.is/c++draft/concepts> にあります（ただし、このリストには C++20 以降に追加された concept が含まれている可能性があります）。

標準ライブラリの concept ``std::assignable_from`` で関数テンプレートを制約する:
...
ネストされた ``MyType::difference_type`` が存在する場合はそれを使用し、そうでない場合は ``std::ptrdiff_t`` を使用したいと仮定します:

まず、``MyType`` がネストされた ``difference_type`` を持つかどうかを検出する concept が必要です:
