...
3つの主な違いは、*固定容量*、*挿入試行の最大回数*、および *インデックスベース* です。

- *固定容量*: 並列アルゴリズム内では、unordered_map の容量は固定されます。
  つまり、マップの容量を超えると、インサートが失敗する可能性があるということです。
  マップの容量はホストからのみ変更(リハッシュ)可能です。

- *挿入試行の最大回数*: 挿入を行う(内部的な)試行が一定回数未満で空き領域が見つからない場合、
  挿入は失敗する可能性があります。これはマップの容量とは無関係に発生することがあります。
...
``UnorderedMap``のために、 ``host_mirror_type`` を作成します。
