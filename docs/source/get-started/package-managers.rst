...
.. note::
   Spack の `deprecated_code` variant は、インストールされたメジャーバージョンに対して非推奨コードを自動的に有効化します（例：バージョン 5.2.0 の場合は `Kokkos_ENABLE_DEPRECATED_CODE_5`）。異なるメジャーバージョンごとの個別の variant は提供されていません。
