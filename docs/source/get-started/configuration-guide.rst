...
Kokkos におけるデバッグの動作は、ビルドタイプと追加のデバッグ CMake オプションの両方によって影響を受けます。
...
**NVCC 固有の事項:** デバイスのデバッグシンボルを完全に取得するには、次のように手動で追加する必要があります。
    * ``CMAKE_CXX_FLAGS="-G"`` を使用し、``CMAKE_CXX_COMPILER`` として ``nvcc_wrapper`` を使用してください。
    * ``Kokkos_ENABLE_COMPILE_AS_CMAKE_LANGUAGE=ON`` でビルドする場合は、代わりに ``CMAKE_CUDA_FLAGS="-G"`` を指定してください。

.. warning:: ``-G`` はほぼすべての GPU 最適化を無効にし、カーネルの実行速度を大幅に低下させます。
...
* * ``Kokkos_ENABLE_DEBUG_BOUNDS_CHECK``
      * 境界チェックを使用 - これにより実行時間が長くなります。CUDA または HIP バックエンドでは、すべてのカーネルの後に同期を行うことを意味します
      * ``OFF``
...
また、推奨される「フルデバッグ」または「ファストデバッグ」の構成については :ref:`FAQ <setup-debug-build>` を参照してください。
