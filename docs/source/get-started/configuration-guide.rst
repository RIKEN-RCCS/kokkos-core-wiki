...
.. list-table::
    ...
    * * ``Kokkos_ENABLE_DEPRECATED_CODE_3``
      * Kokkos 3.x 系列における非推奨のコードの有効化 :red:`[4.3 で削除]`
      * ``OFF``

    * * ``Kokkos_ENABLE_DEPRECATED_CODE_4``
      * Kokkos 4.x 系列における非推奨のコードの有効化 :red:`[5.3 で削除]`
      * ``OFF``
    ...
    * * ``Kokkos_ENABLE_DEPRECATION_WARNINGS``
      * 非推奨の Kokkos コードを使用した際にコンパイル時に警告を発生させるかどうか
      * ``ON``
...
Kokkos におけるデバッグの動作は、ビルドタイプと追加のデバッグ用 CMake オプションの両方によって影響を受けます。

CMake ビルドタイプとコンパイラフラグ:
   * ``CMAKE_BUILD_TYPE=Debug``: 最適化フラグを指定せずに、一般的にデバッグシンボル用のフラグ (``-g``) を有効化します。デフォルトで ``Kokkos_ENABLE_DEBUG`` を有効化します。``KOKKOS_ASSERT`` を有効化します。

   * ``CMAKE_BUILD_TYPE=RelWithDebInfo``: 一般的に最適化フラグ (``-O2``) とともにデバッグシンボル用のフラグ (``-g``) を有効化します。

   **NVCC 固有の事項:** デバイスの完全なデバッグシンボルを取得するには、手動で以下を追加する必要があります。
    * ``CMAKE_CXX_FLAGS="-G"`` を使用し、``nvcc_wrapper`` を ``CMAKE_CXX_COMPILER`` として使用します。
    * ``Kokkos_ENABLE_COMPILE_AS_CMAKE_LANGUAGE=ON`` でビルドする場合は、代わりに ``CMAKE_CUDA_FLAGS="-G"`` を指定します。

   .. warning:: ``-G`` はほぼすべての GPU 最適化を無効化し、カーネルの実行速度を大幅に低下させます。

以下のオプションを使用すると、ビルドタイプに関係なく特定のデバッグ機能を切り替えることができます。
...
.. list-table::
    ...
    * * ``Kokkos_ENABLE_DEBUG_BOUNDS_CHECK``
      * 境界チェックを使用 - これにより実行時間が長くなります。CUDA または HIP バックエンドでは、すべてのカーネルの後に同期が行われます
      * ``OFF``
...
また、推奨される「フルデバッグ」または「高速デバッグ」の構成については :ref:`FAQ <setup-debug-build>` を参照してください。
