...
Kokkos のデバッグ動作は、ビルドタイプと追加のデバッグ用 CMake オプションの両方の影響を受けます:

CMake のビルドタイプとコンパイラフラグ:
   * ``CMAKE_BUILD_TYPE=Debug``: 一般的に、最適化フラグを指定せずにデバッグシンボル用のフラグ (``-g``) を有効にします。デフォルトで ``Kokkos_ENABLE_DEBUG`` を有効にします。``KOKKOS_ASSERT`` を有効にします。

* ``CMAKE_BUILD_TYPE=RelWithDebInfo``: 一般的に、最適化フラグ (``-O2``) とともにデバッグシンボル用のフラグ (``-g``) を有効にします。

**NVCC 固有の事項:** デバイスの完全なデバッグシンボルを取得するには、手動で以下を追加する必要があります
    * ``CMAKE_CXX_FLAGS="-G"`` を使用し、``CMAKE_CXX_COMPILER`` として ``nvcc_wrapper`` を使用します。
    * ``Kokkos_ENABLE_COMPILE_AS_CMAKE_LANGUAGE=ON`` でビルドする場合は、代わりに ``CMAKE_CUDA_FLAGS="-G"`` を指定します。

.. warning:: ``-G`` はほぼすべての GPU 最適化を無効にし、カーネルを大幅に遅くします。
...
* * ``Kokkos_ENABLE_DEBUG``
      * ``KOKKOS_ASSERT`` などの追加のデバッグ機能を有効化 - コンパイル時間が長くなる可能性があります。``nvcc`` でコンパイルする際にコンパイラフラグに ``-lineinfo`` を追加します。
      * ``CMAKE_BUILD_TYPE`` が ``Debug`` の場合 ``ON`` 、それ以外の場合は ``OFF``

* * ``Kokkos_ENABLE_DEBUG_BOUNDS_CHECK``
      * 境界チェックを使用 - これにより実行時間が長くなります。CUDA または HIP バックエンドではすべてのカーネル後に同期が発生します
      * ``OFF``
...
また、推奨される「フルデバッグ」または「高速デバッグ」構成については :ref:`FAQ <setup-debug-build>` を参照してください。
