...
* ``4.2`` 引数は、最低限必要な Kokkos バージョンを指定します。これは任意ですが、互換性を確保するために推奨されています。なお、これによってプロジェクトは次のメジャーリリース（例：``5.0``）を含む、より新しいバージョンをすべて受け入れるように設定されることに注意してください。

  * Kokkos ``4.x`` に厳密に留まる必要がある場合（``5.0`` での破壊的変更を避けるため）、代わりにバージョン範囲を指定する必要があります：

    .. code-block:: cmake

      # Kokkos 4.2 以降を検索するが、5.0 より前で止める
...
.. note::
   ...
   上記の例のように、ソースからのダウンロードおよびビルドにフォールバックする前に、まず `find_package()` で Kokkos を検索しようとする場合、以下のより簡潔な構文を使用できます：
   ...
   `FIND_PACKAGE_ARGS <https://cmake.org/cmake/help/latest/module/FetchContent.html#integrating-with-find-package>`_ キーワードは、``FetchContent`` 経由でダウンロードする前に、まず指定された引数で ``find_package()`` を呼び出そうとするように CMake に指示します。
   常に取得した ``Kokkos`` を使用するには、`OVERRIDE_FIND_PACKAGE <https://cmake.org/cmake/help/latest/module/FetchContent.html#integrating-with-find-package>`_ オプションを追加します：
   ...
   .. code-block:: cmake
      ...
      # 以下は自動的に FetchContent_MakeAvailable() へ転送されます
      find_package(Kokkos 4.4 CONFIG)

   このオプションは、取得した ``Kokkos`` を使用し、以降のすべての ``find_package(Kokkos)`` 呼び出しをバイパスするように CMake に指示します。
   これはまた、``find_package(Kokkos)`` 呼び出しのバージョン要件が無視されることも意味します。
