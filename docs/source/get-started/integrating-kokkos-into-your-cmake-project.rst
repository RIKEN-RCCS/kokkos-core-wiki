...
find_package(Kokkos 4.2 REQUIRED CONFIG) # バージョン 4.2 以降を検索
  # ...
...
* ``4.2`` 引数は、最低限必要な Kokkos バージョンを指定します。これは任意ですが、互換性を確保するために推奨されています。この設定により、次のメジャーリリース（例: ``5.0``）を含め、より新しいバージョンを受け入れるようにプロジェクトが構成される点に注意してください。

* Kokkos ``4.x`` に厳密に留まる必要がある場合（``5.0`` での破壊的変更を避けるため）は、代わりにバージョン範囲を指定する必要があります。

.. code-block:: cmake

# Kokkos 4.2 以降を検索するが、5.0 未満で止める
...
.. note::
   ...
   上記の例のように、ソースからのダウンロードとビルドにフォールバックする前に、まず `find_package()` 経由で Kokkos を検索するには、以下のより簡潔な構文を使用できます。
...
`FIND_PACKAGE_ARGS <https://cmake.org/cmake/help/latest/module/FetchContent.html#integrating-with-find-package>`_ キーワードは、``FetchContent`` 経由でダウンロードする前に、まず指定された引数を使って ``find_package()`` を呼び出すように CMake に指示します。
