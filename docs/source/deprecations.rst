...
* ネストされた OpenMP を有効にせずにネストされた OpenMP の並列を使用すること
   * 置換: なし
   * 非推奨コードなしでコンパイルした際に、バグのあるコードパスの使用を回避します。

* OpenMP 並列領域内での ``OpenMP`` インスタンスの生成
   * 置換: なし
   * パーティションのスレッド数が 0 の場合に ``partition_space`` がアボートするのを回避します。
...
* ``{DynRankView,OffsetView,View}::scalar_array_type``
   ...
   * Kokkos 5 より前の ``View`` 実装を用いた ``{DynRankView, OffsetView, View}`` の特定の外部部分特殊化にのみ関係します。ほとんどの場合 ``data_type`` と等しいです。

* ``{DynRankView,OffsetView,View}::const_scalar_array_type``
   * 置換: ``{DynRankView,OffsetView,View}::const_data_type``
   * Kokkos 5 より前の ``View`` 実装を用いた ``{DynRankView, OffsetView, View}`` の特定の外部部分特殊化にのみ関係します。ほとんどの場合 ``const_data_type`` と等しいです。

* ``{DynRankView,OffsetView,View}::non_const_scalar_array_type``
   * 置換: ``{DynRankView,OffsetView,View}::non_const_data_type``
   * Kokkos 5 より前の ``View`` 実装を用いた ``{DynRankView, OffsetView, View}`` の特定の外部部分特殊化にのみ関係します。ほとんどの場合 ``non_const_data_type`` と等しいです。

* ``{DynRankView,OffsetView,View}::array_type``
   * 置換: ``{DynRankView,OffsetView,View}::type``
   * ``array`` は ``View`` に対する極めて古い参照です。
...
* 静的な ``ExecutionSpace::concurrency()``
   ...
   * 並行性は実行空間インスタンスの属性であり、その型の属性ではありません
