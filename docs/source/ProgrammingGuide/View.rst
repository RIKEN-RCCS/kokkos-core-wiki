...
Kokkos では、すべての割り当てのライフタイムが :cpp:func:`Kokkos::finalize <finalize>` の呼び出しより前に終了することが要求されます。
...
Kokkos はこれらの制限を回避する以下の方法を提供します。まず、ビューには `host_mirror_type` という typedef があり、これは `HostSpace` 内で互換性のあるレイアウトを持つビュー型です。さらに、`host_mirror_type` 型のビューを割り当てる `create_mirror` および `create_mirror_view` 関数があります。両者の違いは、`create_mirror` は常に新しいビューを割り当てるのに対し、`create_mirror_view` は元のビューが `HostSpace` にない場合にのみ新しいビューを作成する点です。
...
Kokkos::View<int*[3], MemorySpace> a ("a", 10);
  // a のレイアウトとパディングで HostSpace 内にビューを割り当てます
  ...
  Kokkos::create_mirror_view(a);
  // MemorySpace が HostSpace の場合、これは no-op です
  Kokkos::deep_copy (c, a)
