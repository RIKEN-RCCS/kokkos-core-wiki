...
メモリ空間とは、データが存在する場所　_Where_ です。 データの物理的な位置と、特定のアクセス特性を指定します。データの物理的な位置と特定のアクセス特性を指定します。 異なる物理的な場所には、高帯域幅メモリ、オンダイスクラッチメモリ、あるいは不揮発性バルクストレージといったものが対応しますが、それは、Host および　GPU　からアクセス可能です。 異なる論理メモリ空間により、CUDAプログラミングモデルにおける UVM メモリのような概念が可能となりますが、これはホストとGPUの両方からアクセスできます。将来的には、メモリ空間を用いてリモートメモリ位置を表現することも可能となるでしょう。 さらに、それらは整合性制御および永続性スコープといった機能を、カプセル化しております
