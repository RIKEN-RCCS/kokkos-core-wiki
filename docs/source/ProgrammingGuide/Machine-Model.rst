...
*Kokkos スレッドは、並列コンピューティングのためのもの* であり、I/Oと計算のオーバーラッピングのためでも、グラフィカルユーザーインターフェースの応答性を高めるためのものでもありません。 後者の二つの目的には、他の種類のスレッド（例：オペレーティングシステムのスレッド）をご利用ください。 コッコスの並列処理を、他の種類のスレッドと組み合わせることが可能かもしれません；詳細については、|Section231|_ を参照してください。 Kokkos の開発者チームは、Kokkos の既存のデータ―並列構造と連携するタスク並列モデルの開発にも取り組んでいます。
