...
.. |HierarchicalParallelism| replace:: 階層的並列処理
...
Kokkos では、ユーザーが実行空間に並列操作を実行するように指示できます。これらには、並列 for、reduce、scan（ |ParallelDispatch|_ を参照）に加え、 |ViewAllocation|_ や |Initialization|_ が含まれます。 このような操作の総称を *並列ディスパッチ* と呼びます。
...
最初のカテゴリが最も制約が厳しいものです。 |Section82|_ では、チーム間同期に関する制約について説明しています。一般的に、Kokkos の並列コードが Kokkos の操作を呼び出す能力は制限されています（ネストされた並列処理を除く。 |HierarchicalParallelism|_ 、特に |Section82|_ を参照）。 また、並列操作における（チームのスクラッチパッドからのものを除く）動的メモリ割り当ても禁止しています。 Kokkos の並列コードがオペレーティングシステムのルーチンやサードパーティのライブラリを呼び出せるかどうかは、使用している実行空間およびメモリ空間に依存します。 いずれにしても、チーム間同期に関する制約は、ファイルシステムへのアクセスなどに影響を及ぼします。
