...
.. |ParallelDispatch| replace:: 並列ディスパッチ
...
Kokkos では、ユーザーが実行空間に並列操作の実行を指示できます。これには、parallel for、reduce、scan（|ParallelDispatch|_ を参照）のほか、|ViewAllocation|_ や |Initialization|_ が含まれます。 このような操作の総称を *並列ディスパッチ* と呼びます。
...
最初のカテゴリーは最も制限が厳しいものです。 |Section82|_ ではチーム間同期に関する制限について説明しています。 一般に、Kokkos の並列コードが Kokkos の操作を呼び出す能力は制限されています（ネストされた並列性の場合を除く；|HierarchicalParallelism|_ 、特に |Section82|_ を参照）。 また、並列操作における（チームのスクラッチパッドからの割り当てを除く）動的なメモリ割り当ても禁止しています。 Kokkos の並列コードがオペレーティングシステムのルーチンやサードパーティのライブラリを呼び出せるかどうかは、使用されている実行空間およびメモリ空間に依存します。 いずれにせよ、チーム間同期に関する制限は、ファイルシステムへのアクセスなどに影響を及ぼします。
