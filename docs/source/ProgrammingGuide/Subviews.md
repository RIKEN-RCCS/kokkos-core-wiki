...
*  　C++11　型推論(`自動`)を使って、Kokkosがサブビュー型を選択できるようにします
...
auto A_sub = サブビュー (A, make_pair (2, 4), make_pair (3, 7));
```
