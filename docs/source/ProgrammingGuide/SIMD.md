...
`Kokkos::Experimental::simd<double>` は、64ビット浮動小数点型の値を格納するベクトルレジスタの基本的なSIMD型です。ポインタ（とアライメントタグ）を指定して構築すると、ベクトル化されたロード命令が実行され、`simd_uncheckd_store` はベクトル化されたストア命令を生成します。多くの場合、これらの命令はCPUからメモリ帯域幅を最大限に引き出す唯一の方法です。SIMD型は基本的な数学演算子を提供するため、3つの乗算と2つの加算は、乗算および加算のベクトル命令に確実に変換されます。`Kokkos::sqrt` のオーバーロードは、CPUがサポートしていれば値の平方根を計算するベクトル命令を呼び出します。
...
```c++
   ...
   このアプローチの主な欠点は、マスク付きのロードおよびストアを使用することによるわずかなオーバーヘッドですが、パディングもデータ型の変更も行っていないソースからのデータを、コードの繰り返しなしにうまく処理できます。
...
これに対する解決策は、`simd_mask` を使ってsimdオブジェクトを構築することです:
...
SIMDの意味で三項条件演算子 `a ? b : c` の動作を模倣する関数を使用するのが一般的な慣行です。これは、以下が機能的に等価であることを意味します:
