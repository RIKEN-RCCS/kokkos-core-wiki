...
`Kokkos::Experimental::simd<double>` は、64ビット浮動小数点型の値を格納するベクトルレジスタのための基本的な SIMD 型です。
ポインタ（およびアライメントタグ）を与えて構築すると、ベクトル化されたロード命令が実行され、`simd_uncheckd_store` はベクトル化されたストア命令を生成します。
多くの場合、これらの命令は CPU から完全なメモリ帯域幅を引き出す唯一の方法です。
SIMD 型は基本的な数学演算子を提供するため、3回の乗算と2回の加算は、乗算と加算のベクトル命令に変換されることが保証されます。
`Kokkos::sqrt` のオーバーロードは、CPU がサポートしていれば、値の平方根を計算するためのベクトル命令を呼び出します。
...
```c++
   ...
   このアプローチの主な欠点は、マスク付きのロードとストアを使用することによるわずかなオーバーヘッドですが、パディングもデータ型の変更も行っていないソースからのデータを、コードの繰り返しなしにうまく処理できます。
...
よくある手法として、三項条件演算子 `a ? b : c` の挙動を SIMD の意味で模倣する関数を使用することがあります。これは、以下が機能的に等価であることを意味します:
