...
Kokkos は、コミュニティに予測可能な更新を提供するため、時間ベースのリリーススケジュールに従っています。

- **メジャーリリース (X.0.0):** 3 年ごと
- **マイナーリリース (X.Y.0):** 4 か月ごと
- **パッチリリース (X.Y.Z):** バグ修正のために必要に応じて

年間リリース周期
~~~~~~~~~~~~~~~~

Kokkos は、**3 月**、**7 月**、**11 月** にリリースを行う、年 3 サイクルのスケジュールに従っています。

.. list-table:: リリースカレンダー
   :header-rows: 1
   :widths: 15 25 60
   ...
   * - **2 月**
     - ブランチの作成
     - 最終週: ``release-candidate-X.Y.0`` ブランチを作成
   * - **3 月**
     - **X.Y.0 リリース**
     - 3 月中旬～下旬 (ブランチ作成の 3 週間後)
   * - **4 月**
     - パッチリリース
     - 必要に応じて X.Y.1 をリリース (ブランチ作成の約 6 週間後)
   * - **5 月**
     - パッチリリース
     - 必要に応じて X.Y.2 以降をリリース
   * - **6 月**
     - ブランチの作成
     - 最終週: 次のリリース候補ブランチを作成
   * - **7 月**
...
各マイナーリリースは、構造化されたタイムラインに従います。

.. code-block:: text

   Week  0: リリースブランチ作成 (2 月/6 月/10 月の最終週)
            - このリリースのフィーチャーフリーズ
            - develop ブランチのバージョンを X.Y.99 に更新

   Week  1-3: テストと安定化フェーズ
              - 重要な修正をチェリーピック
              - 拡張された CI と統合テストを実行
              - changelog とドキュメントを更新

   Week  3: X.Y.0 リリース (3 月/7 月/11 月の中旬～下旬)
            - Git タグを作成
            - GitHub リリースを公開
            - コミュニティに告知
   ...
   Week  6: X.Y.1 リリース (必要に応じて)
            - 蓄積されたバグ修正を含む最初のパッチリリース

   Week  6-17: X.Y.2 以降リリース (必要に応じて)
               - 重要な問題に対する追加パッチ

   Week 17: 次のリリースブランチ作成
            - サイクルの繰り返し (約 4 か月または 17 週間)
...
- バグ修正および機能完成のチェリーピック
- サポートされているプラットフォーム全体での広範なテスト
- ドキュメントの更新

**Week 2-3 (リリース候補後期):**

- **次第に保守的に** - 重要なバグとリグレッションのみに焦点を当てる
- 最終的なドキュメントレビュー
- changelog の確定

**Week 3 以降 (リリース後):**

- パッチリリースはより厳格な基準に従う (バグ修正のみ)
- X.Y.0 との API 互換性を維持

.. note::
   リリーススケジュールは、休暇、重要なバグ、または主要な下流プロジェクトとの調整のために調整される場合があります。
